{
  ...
  outputs = inputs: {
    ...
    nixosModules = {
      ...
      sd-image = ({ lib, ... }: {
        imports = [
          "${inputs.nixpkgs}/nixos/modules/installer/sd-card/sd-image-aarch64-new-kernel-installer.nix"
        ];
      ...
      odroidhc4-uboot = ({ pkgs, lib, ... }: {
        ...
        nixpkgs.config.allowUnfreePredicate = pkg:
          builtins.elem (lib.getName pkg) [ "meson64-tools" ];
        nixpkgs.overlays = [ inputs.self.overlays.odroidhc4-uboot ];
      });
    };
    lib = {
      makeSdImage = nixos:
        nixos.pkgs.buildPackages.symlinkJoin {
          name = "build-${nixos.config.sdImage.imageName}";
          paths =
            [ nixos.config.system.build.sdImage nixos.pkgs.ubootOdroidHC4 ];
        };
    };
    nixosConfigurations = {
      example-hc4 = inputs.nixpkgs.lib.nixosSystem {
        system = "aarch64-linux";
        modules = [ inputs.self.nixosModules.odroidhc4 ];
      };
    };
    packages.aarch64-linux = let
      pkgs = import inputs.nixpkgs {
        ...
        config.allowUnfreePredicate = pkg:
          builtins.elem (inputs.nixpkgs.lib.getName pkg) [ "meson64-tools" ];
      };
    in { uboot-odroid-hc4 = pkgs.ubootOdroidHC4; };
  };
}
